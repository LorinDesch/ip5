...
const TopNavbar = ({
    ...
    const options3 = fakeData.groups.map((item) => ({value: item.groupname, label: item.groupname,}));
    ...
    return (<div className="top-navbar">
        {windowWidth < 768 && (
            <button className="menu-button" onClick={toggleMenu}>
                <div className={`hamburger ${isMenuOpen ? 'open' : ''}`}><span></span> <span></span> <span></span></div>
            </button>)}
        {(windowWidth >= 768 || isMenuOpen) && (
            <>
                <Dropdown1
                    ...
                    fakeData={fakeData}/>
                <Dropdown2
                    options={options2} onChange={handleChange2}
                    styling={windowWidth < 768 ? smallDropDownStyling : dropDownStyling}
                    selectedOptions2={selectedOption2} fakeData={fakeData}/>
                <button onClick={handleLogout} className="logout-button"> logout</button>
                <Dropdown3
                    options={options3} onChange={handleChange3}
                    styling={windowWidth < 768 ? smallDropDownStyling : dropDownStyling}
                    selectedOptions={selectedOption3} fakeData={fakeData}/>
            </>
        )}
    </div>);
};
